        ...
        with requests_html_playwright.HTMLSession() as session:
            ...
            while active_queue:
                for key in active_queue.keys():
                    ...
                    try:
                        page.wait_for_selector("#jsonlResults", timeout=30000)
...
def main(fasta_path: str, database: int, operating_mode: int) -> None:
    ...
    # check if there is an old download queue
    try:
    ...
    while download_queue["waiting"] or download_queue["active"]:
        ...
        if len(download_queue["active"]) < 5 and download_queue["waiting"]:
            ...
            request_id, current_request_object = download_queue["waiting"].popitem(
                last=False
            )
            tqdm.write(
    ...
    # finally check if all downloads were finished and are saved


main("C:\\Users\\Dominik\\Documents\\GitHub\\BOLDigger3\\tests\\test_1000.fasta", 3, 3)
